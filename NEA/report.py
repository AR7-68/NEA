...
class Report(): # Defining a class
    def __init__(self, root):
        ...
        # title
        title = Label(self.root, text = 'Report', font = ('goudy old style', 15, 'bold'), bg = '#0f4d7d', fg = 'white').place(x = 50, y = 10, width = 1000, height = 40) # Decode title
        ...
        # 4th Row
        lbl_success = Label(self.root, text = 'Success', font = ('goudy old style', 15), bg = '#242424', fg = 'white').place(x = 50, y = 200) # Product Label
        self.success = Entry(self.root, textvariable = self.var_success, font = ('goudy old style', 15), bg = 'lightblue') # Product entry
        self.success.place(x = 180, y = 200, width  = 180)
        ...
        self.reportTable = ttk.Treeview(emp_frame, columns = ('id', 'category', 'dtype', 'product', 'success'), yscrollcommand = scrolly.set, xscrollcommand = scrollx.set)
        ...
        self.reportTable.heading('category', text = 'Category')  
        self.reportTable.heading('dtype', text = 'dtype')  
        self.reportTable.heading('success', text = 'success') 
        self.reportTable['show'] = 'headings'
        self.reportTable.column('id', width = 100)  
        self.reportTable.column('category', width = 100)  
        self.reportTable.column('dtype', width = 100)  
        self.reportTable.column('success', width = 100)   
...
#================================================================================================================================================================================================================================#
    def add(self):
        ...
        try:
            if self.var_id.get()=='': # Checks to see if input is empty
                ...
                cur.execute('Select * from report where id=?', (self.var_id.get(),))
                row = cur.fetchone()
                if row != None: # Checks if ID already exists
                    messagebox.showerror('Error', 'ID number is already assigned', parent = self.root)
                else:
                    cur.execute("Insert into report (id, category, dtype, success) values(?,?,?,?)",(
                                                self.var_id.get(),
                                                self.var_category.get(),
                                                self.var_dtype.get(),
                                                self.var_success.get(),
    ...
    def show(self):
        ...
        try:
            ...
            self.reportTable.delete(*self.reportTable.get_children()) # Deletes old data
            for row in rows:
                self.reportTable.insert('', END, values = row) # Displays all data including new inputted data
        except Exception as ex:
            messagebox.showerror('Error',f'Error due to : {str(ex)}', parent = self.root)                    

    # Show all the given data in the entry boxes
    def get_data(self, ev): # Define get_data function
        f = self.reportTable.focus() # Shows the selected row
        content = (self.reportTable.item(f)) # Stores the selected row
        row = content['values'] # Returns the selected row in the input boxes
        print(row)
        self.var_id.set(row[0]),
        self.var_category.set(row[1]),
        self.var_dtype.set(row[2]),
        self.var_success.set(row[3]),
        ...
        try:
            if self.var_id.get()=='': # Validates input
                ...
                if row == None:
                    ...
                    cur.execute("Update report set category=?, dtype=?, success=? where id=?",(
                                                self.var_category.get(),
                                                self.var_dtype.get(),
                                                self.var_success.get(),
                                                self.var_id.get(),
        ...
        try:
            if self.var_id.get()=='':
                ...
                if row == None: 
                    ...
                    if op == True:
                        cur.execute('Delete from report where id=?', (self.var_id.get(),))
                        con.commit()
    ...
    def search(self):
        ...
        try:
           if self.var_searchtxt.get() == '': # Validates if user has entered anything
               ...
               cur.execute('select * from report where id=?',(self.var_searchtxt.get(),))
               row = cur.fetchone()
               if row != None:
                    self.reportTable.delete(*self.reportTable.get_children()) # Removes data that doesn't match with inputs
                    self.reportTable.insert('', END, values = row) # Inserts data that matches input into treeview 
               else:
...
if __name__=='__main__':      
    ...
    obj=Report(root) # Create object of the class  
